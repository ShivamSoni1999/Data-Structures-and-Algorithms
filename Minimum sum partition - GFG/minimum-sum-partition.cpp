...
class Solution{
  ...
  public:
    vector < int > subsetsum(int arr[], int n, int sum) {
      ...
      vector < int > v;                 // contains all subset sums possible with n elements of last row
      for (int j = 0; j <= sum; j++) {  //from the range(sum) we need to exclude the element which is not there in the existing problem
